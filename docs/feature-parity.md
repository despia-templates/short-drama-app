...
| **Rewards** `Rewards.dsx` | The reference page (spec §9), two `<pager>` tabs: **Reward coins** — balance hero, 7-cell check-in card (curve 10·20·20·10·10·25·40, `parts/CheckinCard.dsx`), Earn Rewards rows (`parts/RewardRow.dsx`, `parts/FollowRow.dsx`): sign-in +60 (granted by the provider on account link), push +60 (on the token, `POST /viewer/push`), rewarded ad (15/day × 20 via `parts/AdGate.dsx` — **VIP-gated**, App Store 2.5.18 report seam), watch 5/10 min (the mission ledger), invite +50 per verified invitee up to 10/day (`/invite/:code`, `parts/ReferralSeam.dsx`, a bounded resumable sweep), follow/share (no coins, unverifiable), spin wheel (1/day, server-declared prize table); **Member Points** — membership timeline (`parts/MembershipTimeline.dsx`), Daily Draw (5–50 points, members), Watch & Earn (+50 at 30 min), recommendations, push row (+30 points), Redeem Points → membership days (`POST /rewards/redeem`, idempotent per day+card, `parts/RedeemCard.dsx`), the Coming Soon band (`parts/ComingSoonRail.dsx`, shared with Home); the success modal (`parts/RewardsSuccess.dsx`), Rules sheet (`parts/RulesSheet.dsx`) and enable-notification modal (`parts/NotifyModal.dsx`) |
