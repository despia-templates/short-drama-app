...
| 5 | Bonus coins are promised to expire and never do | S1 · `server/wallet.dsx:22` + `server/engage.dsx:108` |
...
- **S1** — compute the live bonus balance from unexpired ledger rows in `walletState`, and spend
  against that in both unlock actions.
